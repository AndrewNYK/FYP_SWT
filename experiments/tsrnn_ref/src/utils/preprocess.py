...
def reshape_patch(img_tensor, patch_size):
  ...
  b = np.transpose(a, [0, 1, 4, 2, 3, 5, 6])  # by lyz: w h exchange
  patch_tensor = np.reshape(b, [
      batch_size, seq_length, img_width // patch_size, img_height // patch_size,
...
def reshape_patch_back(patch_tensor, patch_size):
  ...
  a = np.reshape(patch_tensor, [
      batch_size, seq_length, patch_width, patch_height, patch_size, patch_size,
      img_channels
  ])
  b = np.transpose(a, [0, 1, 3, 4, 2, 5, 6])
